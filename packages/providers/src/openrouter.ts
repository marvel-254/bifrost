...
export type OpenRouterProviderConfig = { apiKey?: string; baseUrl?: string; defaultModel?: string; timeoutMs?: number };
...
export class OpenRouterProvider implements IProvider {
  ...
  constructor(config: OpenRouterProviderConfig = {}) {
    ...
    this.defaultModel = config.defaultModel || 'meta-llama/llama-3.3-70b-instruct:free'; this.timeoutMs = config.timeoutMs || DEFAULT_TIMEOUT_MS;
  }

  async authenticate(): Promise<boolean> { if (!this.apiKey) return false; try { await this.fetchJson(`${this.baseUrl}/models`); return true; } catch { return false; } }
  async listModels(): Promise<Array<{ id: string; name: string; contextWindow: number; capabilities: string[] }>> {
    const res = await this.fetchJson<{ data: Array<{ id: string; context_length?: number; architecture?: { modality?: string } }> }>(`${this.baseUrl}/models`);
  ...
  async complete(request: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    if ((res as any).error) throw new Error(String((res as any).error?.message || 'OpenRouter error'));
    return res;
  }
  async stream(request: Record<string, unknown>, onChunk: (chunk: Record<string, unknown>) => void | Promise<void>): Promise<void> {
    ...
    if (!res.ok) { const t = await res.text(); throw new Error(`OpenRouter stream error: ${res.status} ${t}`); }
    ...
    while (true) { const { done, value } = await reader.read(); if (done) break; buf += dec.decode(value, { stream: true }); const lines = buf.split('\n'); buf = lines.pop() || '';
      for (const line of lines) { const t = line.trim(); if (!t.startsWith('data: ')) continue; const d = t.slice(6).trim(); if (d === '[DONE]') continue;
        try { const p = JSON.parse(d); const c = p.choices?.[0]; if (c?.delta?.content) await onChunk({ id: p.id, object: 'chat.completion.chunk', created: p.created, model: p.model, choices: [{ index: 0, delta: { content: c.delta.content }, finish_reason: c.finish_reason || null }] }); } catch { /* skip */ } } }
  }
  async healthCheck(): Promise<{ healthy: boolean; latencyMs?: number; error?: string }> {
    const s = Date.now(); try { await this.fetchJson(`${this.baseUrl}/models`); return { healthy: true, latencyMs: Date.now() - s }; } catch (e) { return { healthy: false, error: e instanceof Error ? e.message : String(e) }; }
  }
  ...
  private async fetchJson<T>(url: string, init?: RequestInit): Promise<T> {
    ...
    if (!res.ok) { const t = await res.text(); throw new Error(`OpenRouter error ${res.status}: ${t}`); }
    return res.json() as Promise<T>;
  }
}

export function createOpenRouterProvider(config?: OpenRouterProviderConfig): OpenRouterProvider { return new OpenRouterProvider(config); }
