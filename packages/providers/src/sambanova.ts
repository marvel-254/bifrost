...
export type SambaNovaProviderConfig = { apiKey?: string; baseUrl?: string; defaultModel?: string; timeoutMs?: number };

export class SambaNovaProvider implements IProvider {
  name = 'sambanova';
  ...
  constructor(config: SambaNovaProviderConfig = {}) {
    ...
    this.defaultModel = config.defaultModel || 'Meta-Llama-3.3-70B-Instruct'; this.timeoutMs = config.timeoutMs || DEFAULT_TIMEOUT_MS;
  }
  ...
  async complete(request: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    if ((res as any).error) throw new Error(String((res as any).error?.message || 'SambaNova error'));
    return res;
  }
  async stream(request: Record<string, unknown>, onChunk: (chunk: Record<string, unknown>) => void | Promise<void>): Promise<void> {
    ...
    if (!res.ok) { const t = await res.text(); throw new Error(`SambaNova stream error: ${res.status} ${t}`); }
  ...
  private async fetchJson<T>(url: string, init?: RequestInit): Promise<T> {
    ...
    if (!res.ok) { const t = await res.text(); throw new Error(`SambaNova error ${res.status}: ${t}`); }
    return res.json() as Promise<T>;
  }
}

export function createSambaNovaProvider(config?: SambaNovaProviderConfig): SambaNovaProvider { return new SambaNovaProvider(config); }
