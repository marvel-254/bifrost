...
export type MistralProviderConfig = { apiKey?: string; baseUrl?: string; defaultModel?: string; timeoutMs?: number };
...
export class MistralProvider implements IProvider {
  ...
  constructor(config: MistralProviderConfig = {}) {
    ...
    this.defaultModel = config.defaultModel || 'mistral-small-latest'; this.timeoutMs = config.timeoutMs || DEFAULT_TIMEOUT_MS;
  }

  async authenticate(): Promise<boolean> { if (!this.apiKey) return false; try { const r = await this.fetchJson<{ object: string }>(`${this.baseUrl}/models`); return !!r; } catch { return false; } }
  async listModels(): Promise<Array<{ id: string; name: string; contextWindow: number; capabilities: string[] }>> {
    const res = await this.fetchJson<{ data: Array<{ id: string; capabilities?: string[] }> }>(`${this.baseUrl}/models`);
  ...
  async complete(request: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    if ((res as any).error) throw new Error(String((res as any).error?.message || 'Mistral error'));
    return res;
  }
  async stream(request: Record<string, unknown>, onChunk: (chunk: Record<string, unknown>) => void | Promise<void>): Promise<void> {
    ...
    if (!res.ok) { const t = await res.text(); throw new Error(`Mistral stream error: ${res.status} ${t}`); }
  ...
  private async fetchJson<T>(url: string, init?: RequestInit): Promise<T> {
    ...
    if (!res.ok) { const t = await res.text(); throw new Error(`Mistral error ${res.status}: ${t}`); }
    return res.json() as Promise<T>;
  }
}

export function createMistralProvider(config?: MistralProviderConfig): MistralProvider { return new MistralProvider(config); }
