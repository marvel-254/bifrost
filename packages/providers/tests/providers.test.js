...
function mockOllamaApi(tagsResponse, chatResponse) {
...
describe('OllamaProvider', () => {
    let provider;
    beforeEach(() => {
        provider = new index_1.OllamaProvider({ baseUrl: 'http://mock-ollama:11434' });
        mockOllamaApi();
    });
    afterEach(() => {
        restoreFetch();
    ...
    describe('constructor', () => {
        test('creates with defaults', () => {
            const p = new index_1.OllamaProvider();
            expect(p).toBeInstanceOf(index_1.OllamaProvider);
        });
        test('custom baseUrl', () => {
            const p = new index_1.OllamaProvider({ baseUrl: 'http://custom:11434' });
            expect(p).toBeInstanceOf(index_1.OllamaProvider);
        });
    });
    describe('name', () => {
    ...
    describe('complete (non-streaming)', () => {
        ...
        test('throws on API error', async () => {
            ...
            await expect(provider.complete({ model: 'bad', messages: [{ role: 'user', content: 'hi' }] })).rejects.toThrow('Model not found');
        });
    });
    describe('stream', () => {
        test('yields streamed chunks', async () => {
            const chunks = [];
            await provider.stream({ model: 'llama3', messages: [{ role: 'user', content: 'Hi' }] }, (chunk) => { chunks.push(chunk); });
        ...
        test('streaming chunks have correct structure', async () => {
            ...
            const choices = firstChunk.choices;
        ...
        test('throws on stream error', async () => {
            ...
            await expect(provider.stream({ model: 'llama3', messages: [{ role: 'user', content: 'Hi' }] }, () => { })).rejects.toThrow('Ollama stream error');
            restoreFetch();
        });
    });
...
describe('ProviderRegistry', () => {
    test('starts empty', () => {
        const registry = new index_1.ProviderRegistry();
