...
export function selectFastest(candidates: ScoredCandidate[]): StrategyResult {
  if (candidates.length === 0) {
    return { primary: null, fallbacks: [] };
  }

  const sorted = [...candidates].sort((a, b) => b.candidate.latencyScore - a.candidate.latencyScore);
  return {
